...
export default function SignIn({ history }) {
  ...
  const handleSignUp = useCallback(async event => {
    ...
    try {
      await FirebaseAuth.doCreateUserWithEmailAndPassword(email.value,password.value);
      history.push("Home");
    } catch (error) {
      alert(error);
    }
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row items-center md:items-start sm:justify-center md:justify-start flex-auto min-w-0 bg-white">
          <div className="sm:w-1/2 xl:w-3/5 h-full hidden md:flex flex-auto items-center justify-center p-10 overflow-hidden bg-purple-900 text-white bg-no-repeat bg-cover relative"
            style={{backgroundImage: "url(" + "https://www.clipartmax.com/png/middle/364-3641039_clipart-black-and-white-stock-work-at-home-remote-jobs-woman.png " + ")"}}>
